...
export default function BottomNavGlass({
  items,
  activePath,
  onItemClick,
  className,
    ...
    >
      ...
      >
        {items.map((item, index) => {
          ...
          if (isCenter) {
            return (
              <li key={item.path} className="glass-nav-item relative z-10 flex h-[85px] items-center justify-center">
                <button
          ...
          return (
            <li key={item.path} className="glass-nav-item relative z-[1] flex h-[85px] items-center justify-center">
              <button
                aria-current={isActive ? "page" : undefined}
                className={cn(
