...
const ambientGlowColors: Record<string, string> = {
  home: "radial-gradient(circle, rgba(59,130,246,0.45), rgba(99,102,241,0.25), transparent 70%)",
  analytics: "radial-gradient(circle, rgba(16,185,129,0.45), rgba(20,184,166,0.25), transparent 70%)",
  wallet: "radial-gradient(circle, rgba(245,158,11,0.45), rgba(249,115,22,0.25), transparent 70%)",
  alerts: "radial-gradient(circle, rgba(217,70,239,0.45), rgba(236,72,153,0.25), transparent 70%)",
  settings: "radial-gradient(circle, rgba(100,116,139,0.45), rgba(71,85,105,0.25), transparent 70%)",
};
...
export function HeroPhoneMockup({ isLight, activeId, onNavItemClick }: HeroPhoneMockupProps) {
  ...
  return (
    <div className={cn("relative mx-auto w-[760px] border rounded-[1.5rem] dark:border-white/10 max-w-full flex-none overflow-hidden hero-phone-float", shellHeight)}>
      {isLight ? (
        <div
          aria-hidden="true"
      ...
      {/* Ambient glow — shifts color with active screen tab */}
      <div
        aria-hidden="true"
        className={cn("hero-ambient-glow transition-opacity duration-500", isLight ? "opacity-0" : "opacity-100")}
        style={{ background: ambientGlowColors[activeId] ?? ambientGlowColors.home }}
      />
      ...
      >
        ...
        >
          ...
          />
            <div
              ...
              className={cn(
                "hero-dynamic-island",
                isLight
                  ? "bg-black shadow-[inset_0_1px_1px_rgba(255,255,255,0.06)]"
                  : "bg-[#1a1a1c] shadow-[inset_0_1px_1px_rgba(255,255,255,0.05)]"
              )}
            >
              {/* Front camera lens */}
              <div
                className={cn(
                  "absolute right-[14px] top-1/2 -translate-y-1/2 h-[7px] w-[7px] rounded-full",
                  isLight
                    ? "bg-[#1a1a2e]/30 ring-[0.5px] ring-black/10"
                    : "bg-[#2a2a2e] ring-[0.5px] ring-white/5"
                )}
              />
            </div>
          ...
          >
            ...
            <div className="absolute inset-x-0 bottom-0 z-20 px-4 [&>div]:absolute [&>div]:bottom-6">
              <BottomNavLiquid
                items={NAV_ITEMS}
                activeId={activeId}
